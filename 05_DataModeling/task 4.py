...
target_corr = corr_spearman['lung_cancer_risk']

# =========================================================
# AGGRESSIVE FEATURE SELECTION (Optimize for Speed)
# =========================================================
# Drop all columns that have absolute correlation < 0.6
# This will drop weak AND moderate correlation features
weak_mod_corr_cols = target_corr[target_corr.abs() < 0.45].index.tolist()

# Drop redundant smoking columns
redundant_cols = ['smoker', 'smoking_years', 'cigarettes_per_day']

cols_to_drop = list(set(weak_mod_corr_cols + redundant_cols))

# Make sure target variable is not accidentally dropped
if 'lung_cancer_risk' in cols_to_drop:
    cols_to_drop.remove('lung_cancer_risk')

df_ultra_reduced = df.drop(columns=cols_to_drop)

print(f"Original dataset: {df.shape[1]} columns. Ultra-Reduced dataset: {df_ultra_reduced.shape[1]} columns.")
print(f"Features used for training: {list(df_ultra_reduced.columns[:-1])}\n")
...
for i in range(1, 4):
    rs_shuffle = i * 24
    df_shuffled = df_ultra_reduced.sample(frac=1, random_state=rs_shuffle).reset_index(drop=True)
...
w_fx = max(len("Summarized f(x)"), max(len(r['f_x']) for r in results)) + 2
...
print(divider)
print_row("Iteration", "Dataset", "Summarized f(x)", "Accuracy")
print(divider)
for r in results[:-1]:
...
# 5. Lưu hoặc hiển thị
plt.tight_layout()
plt.savefig('accuracy_charttask4.png') # Lưu thành file ảnh
